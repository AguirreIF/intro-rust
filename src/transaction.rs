...
impl Amount {
    pub fn from_sats(satohis: u64) -> Amount {
        Amount(satohis)
    }
}
...
impl Decodable for Version {
    fn consensus_decode<R: Read>(r: &mut R) -> Result<Self, Error> {
        Ok(Version(u32::consensus_decode(r)?))
    }
}
...
impl Encodable for Vec<TxIn> {
    fn consensus_encode<W: Write>(&self, w: &mut W) -> Result<usize, Error> {
        let mut len = CompactSize(self.len() as u64).consensus_encode(w)?;
        for tx_in in self {
            len += tx_in.consensus_encode(w)?;
        }
        Ok(len)
    }
}
...
impl Encodable for TxIn {
    fn consensus_encode<W: Write>(&self, w: &mut W) -> Result<usize, Error> {
        let mut len = self.previous_txid.consensus_encode(w)?;
...
impl Encodable for String {
    fn consensus_encode<W: Write>(&self, w: &mut W) -> Result<usize, Error> {
        // TODO
        let b = hex::decode(self).unwrap();
        let compact_size_len = CompactSize(b.len() as u64).consensus_encode(w)?;
        let b_len = w.write(b.as_slice()).map_err(Error::Io)?;
...
impl Encodable for CompactSize {
    fn consensus_encode<W: Write>(&self, w: &mut W) -> Result<usize, Error> {
        match self.0 {
            ...
            0xFD..=0xFFFF => {
                w.write(&[0xFD]).map_err(Error::Io)?;
                (self.0 as u16).consensus_encode(w)?;
                Ok(3)
            }
            0x10000..=0xFFFFFFFF => {
                w.write(&[0xFE]).map_err(Error::Io)?;
                (self.0 as u32).consensus_encode(w)?;
                Ok(5)
            }
            _ => {
                w.write(&[0xFF]).map_err(Error::Io)?;
